...
# 배포된 스크립트 버전 확인 (반드시 self-heal-v7 이상이 보여야 함)
...
로그에 `version=2026-02-11-self-heal-v7`가 보여야 최신 스크립트가 실제로 실행된 것입니다.
...
그리고 updater가 최신(`self-heal-v7`)인지 확인 후 재시작합니다.
...
참고: 최신 updater(`self-heal-v7`)는 시작 시 오염 KV 값 탐지 시 자동 삭제를 시도합니다.
