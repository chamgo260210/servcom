...
# 배포된 스크립트 버전 확인 (반드시 strict-host-v5 보여야 함)
...
로그에 `version=2026-02-11-strict-host-v5`가 보여야 최신 스크립트가 실제로 실행된 것입니다.
...
최신 스크립트는 429 감지 시 백오프 재시도를 수행하며, 장시간 429가 지속되면 code 79로 종료해 재시작 폭주를 막습니다.
...
그리고 updater가 최신(`strict-host-v5`)인지 확인 후 재시작합니다.
